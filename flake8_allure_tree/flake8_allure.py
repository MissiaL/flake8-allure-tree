...
def _get_required_decorators(node, decorators, strict_tree):
    ...
    allure_decorators = [
        ...
        if isinstance(d, ast.Call) and getattr(getattr(d.func, 'value', None), 'id', None) == 'allure'
    ]
...
class MyConfig:
    def __init__(self, class_tag_tree, method_tag_tree, test_class_name, test_method_name, strict_tree):
...
class FlakeClassAllurePytestVisitor(Visitor):
    ...
    def visit_ClassDef(self, node) -> None:
        if self._is_test_class(node):
            required_decorators = _get_required_decorators(node, self.config.class_tag_tree, self.config.strict_tree)
    ...
    def visit_ClassDef(self, node) -> None:
        for method in node.body:
            if self._is_test_method(method):
                required_decorators = _get_required_decorators(method, self.config.method_tag_tree,
                                                               self.config.strict_tree)
                if required_decorators:
                    self.error_from_node(
...
class AllurePytestPlugin(Plugin):
    ...
    def add_options(cls, options_manager: OptionManager):
        ...
        options_manager.add_option('--test-class-name', default='Test*', help='Support glob pattern')
        options_manager.add_option('--test-method-name', default='test_*', help='Support glob pattern')
        options_manager.add_option('--strict-tree', action='store_true',
